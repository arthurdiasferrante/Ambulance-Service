...
class NeighborhoodHospitalRouterTest {

    @Test
    void nearestHospitalWithAvailableBed_matchesExpectedScenario() {
        ...
        for (int i = 0; i < origins.length; i++) {
            assertEquals(expected[i], router.findHospital(origins[i]), "origin " + origins[i]);
        }
    }
}
